...
#define TOUCHSCREEN_MINY 	500
#define TOUCHSCREEN_MAXY 	7700//8200//7669

#define CFG_MAX_POINT_NUM	5

static bool  point_status[CFG_MAX_POINT_NUM];
...
static int ts_i2c_early_suspend(struct early_suspend *h);
static int ts_i2c_late_resume(struct early_suspend *h);
#endif
			...
			for(j=0; j<CFG_MAX_POINT_NUM; j++)
			{
				...
				for(i=0; i<Rdbuf[0]; i++)
				{
					if(j == Rdbuf[i*5+6])
					{
						...
						input_mt_slot(tsdata->input, Rdbuf[i*5+6]);
...
//						printk("x %d  y%d\n",posx[i],posy[i]);	
				...
				if((is_down == false)&&(point_status[j] == true))	
				{
					input_mt_slot(tsdata->input, j);
					input_mt_report_slot_state(tsdata->input, MT_TOOL_FINGER, false);
...
//	printk("ts_i2c_probe\n");
	...
	input_mt_init_slots(input, CFG_MAX_POINT_NUM);
...
//	i2c_set_clientdata(client, &tsdata);
    ...
    tsdata->early_suspend.suspend = ts_i2c_early_suspend;
    tsdata->early_suspend.resume = ts_i2c_late_resume;
...
int realarm_ts_attach_adapter(void)
{
