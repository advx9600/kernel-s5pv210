...
#define GPIO_RESET			S5PV210_GPJ2(1)		
#define IRQ					gpio_to_irq(GPIO_INT)//IRQ_EINT(20)	
...
int init_platform_hw(void)
{
	...
	s3c_gpio_setpull(GPIO_RESET,S3C_GPIO_PULL_UP);
...
*/
	...
	{
		if(tsp_keystatus[i] == KEY_PRESS)
		{
			input_event(data->input_dev, EV_KEY, ((int*)data->input_dev->keycode)[i], 0);
			i_need_sync = 1;
	...
	if(touch_point_num != 0)
	{
		...
		if(ret >= 0)
		{
			do
			{
				...
				{
//				printk("id %d  x %d  y %d\n",id,_st_finger_infos[i_count].i2_x,_st_finger_infos[i_count].i2_y);					
					if(_st_finger_infos[i_count].i2_x > SCREEN_MAX_X)
					{
						if(_st_finger_infos[i_count].i2_y>450)
						{
								input_event(data->input_dev, EV_KEY, ((int*)data->input_dev->keycode)[0], 1);
								tsp_keystatus[0] = KEY_PRESS;						
						}
						else if(_st_finger_infos[i_count].i2_y>400)
						{
								input_event(data->input_dev, EV_KEY, ((int*)data->input_dev->keycode)[1], 1);
								tsp_keystatus[1] = KEY_PRESS;						
						}
						else if(_st_finger_infos[i_count].i2_y>350)
						{
								input_event(data->input_dev, EV_KEY, ((int*)data->input_dev->keycode)[2], 1);
								tsp_keystatus[2] = KEY_PRESS;						
						}
					}
					else
					{
...
*/	
	this_client = client;
	err = i2c_smbus_read_i2c_block_data(this_client, FT5X0X_REG_FIRMID, sizeof(reg_version), &reg_version);
	if (err < 0) {
...
//	client->irq = gpio_to_irq(client->irq);
	...
	i2c_smbus_read_i2c_block_data(this_client, FT5X0X_REG_FIRMID, sizeof(reg_version), &reg_version);
	printk("[LAIBAO] firmware version = 0x%2x\n", reg_version);	

	i2c_smbus_read_i2c_block_data(this_client, FT5X0X_REG_REPORT_RATE, sizeof(reg_value), &reg_value);
	printk("[LAIBAO] firmware report rate = %dHz\n", reg_value*10);	

	i2c_smbus_read_i2c_block_data(this_client, FT5X0X_REG_THRES, sizeof(reg_value), &reg_value);
	printk("[LAIBAO] firmware threshold = %d\n", reg_value * 4);
	
	i2c_smbus_read_i2c_block_data(this_client, FT5X0X_REG_NOISE_MODE, sizeof(reg_value), &reg_value);
	...
	err = request_irq(_sui_irq_num, fts_ts_irq, IRQF_TRIGGER_FALLING, client->dev.driver->name, ft5x0x_ts);
...
int ts_attach_adapter(void)
{
	...
	strlcpy(info.type, "laibao_touch", I2C_NAME_SIZE);
