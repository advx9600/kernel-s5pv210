...
static int NVTtouch_init_panel(struct NVTtouch_ts_data *ts)
{
	...
	if((!ts->abs_x_max)||(!ts->abs_y_max)||(!ts->max_touch_num) || (ts->max_touch_num >20))
	{
...
//	input_sync(ts->input_dev);
	...
	run_count = ts->max_touch_num;
  	for (index = 0; index < ts->max_touch_num; index++) //0~9 (10 points)
  	{
		...
		else if ( (point_data[position]&0x03) == 0x03 || (point_data[position]&0x03) == 0x00 )
			run_count--;
	}
	...
	if(run_count)
	{
		for (index = 0; index < ts->max_touch_num; index++) //0~9 (10 points)
		{
			...
			else 
			{
				...
				input_x=input_x*SCREEN_WIDTH/TOUCH_MAX_WIDTH;
				input_y=input_y*SCREEN_HEIGHT/TOUCH_MAX_HEIGHT;
				input_report_abs(ts->input_dev, ABS_MT_POSITION_X, input_x);
				input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, input_y);
				input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, input_w);
...
{
//	printk("touch screen irq interrupted\n");
...
#endif
	if (ts->client != NULL && ts->client->irq != NULL){ // nwz
...
//	printk("gpio_to_irq(S5PV210_GPH2(3)):%d\n",gpio_to_irq(S5PV210_GPH2(3)));
	//printk("gpio_to_irq(S5PV210_GPH2(4)):%d\n",gpio_to_irq(S5PV210_GPH2(4)));
...
//	.irq = S5PV210_GPH2(4)//IRQ_EINT(20)
	.irq = IRQ_EINT(19)
};
